...
if brick then
  ...
  local freshOptions = {}
  T.check(brick.ensureVoxelDefault(freshOptions),
          "fresh options receive a VOXEL default")
  T.eq(freshOptions.pipelines.voxel, 2,
       "fresh VOXEL options default to MEDIUM")
  local storedOptions = { pipelines = { voxel = 1 } }
  T.check(not brick.ensureVoxelDefault(storedOptions),
          "stored VOXEL selection is not migrated")
  T.eq(storedOptions.pipelines.voxel, 1,
       "stored HIGH selection remains HIGH")
...
if MeshCache and MeshCache.encodeMesh then
  ...
  local sd, latchIdx
  for i = 1, 12 do
    local n, v = debug.getupvalue(MeshCache.dir, i)
    if n == "SaveData" then sd = v end
    if n == "dirTried" then latchIdx = i end
  end
  local origPB = sd and sd.portableBaseDir
  if origPB then sd.portableBaseDir = function() return "/tmp/dsm_dir_test" end end
  if latchIdx then debug.setupvalue(MeshCache.dir, latchIdx, false) end
  local okD, d = pcall(MeshCache.dir)
  if origPB then sd.portableBaseDir = origPB end
  ...
  -- Desktop fallback: with portableBaseDir nil (a non-portable desktop),
  -- dir() must fall back to the LÖVE save directory -- the same io-rooted
  -- location the engine saves to -- without throwing. (In the headless
  -- harness the love stub's getSaveDirectory is /tmp/pokeport-stub-save,
  -- so the cache resolves under that.)
  if sd then
    sd.portableBaseDir = function() return nil end
    if latchIdx then debug.setupvalue(MeshCache.dir, latchIdx, false) end
  ...
  if ffi then
    ...
    for i = 1, 12 do
      local n, v = debug.getupvalue(MeshCache.dir, i)
      if n == "SaveData" then rtsd = v end
      if n == "dirTried" then rtLatch = i end
    end
    local rtBase = "/tmp/dsm_roundtrip"
    os.execute('rm -rf "' .. rtBase .. '"')
    local rtPB = rtsd and rtsd.portableBaseDir
    if rtPB then rtsd.portableBaseDir = function() return rtBase end end
    ...
    os.execute('rm -rf "' .. rtBase .. '"')
    if rtPB then rtsd.portableBaseDir = rtPB end
  end
end
