...
- **Deterministic Brick HIGH shadow map.** When the Brick profile is active
  (`DS_BRICK=1` or unset), HIGH now always fits a 1536x1536 shadow map;
  MEDIUM/LOW/POTATO retain the reduced adaptive ladder, while desktop keeps
  its 1024/1536/2048 ladder.
- **Android cache prebuild feedback.** PREBUILD MAP CACHE now shows compact
  progress and an ETA while it runs on Android, and the active row is labeled
  CANCEL PREBUILD for a clear touch-friendly cancellation action. Desktop and
  Brick status text and READY rebuild confirmation are unchanged.
- **POTATO upscaler replaced.** The default is now `brick_fast`, a single-pass,
  one-fetch polynomial remap tuned for the TrimUI Brick PowerVR GE8300. It
  avoids `pow()` and extra passes while preserving transparent voids and
  premultiplied-alpha correctness. MEDIUM/LOW keep their existing upscale plus
  RCAS path; the desktop Pixel AA variants remain available through
  `DS_UPSCALE=paa|paa_sp` for comparison.

## 1.2.0 — 2026-08-08

### Performance Optimizations

- **In-place Matrix Math (`Mat4.lua`)**: Added in-place matrix operations (`mulInPlace`, `translateInPlace`, `scaleInPlace`, `rotateXInPlace`, `rotateYInPlace`) and pre-allocated static scratch buffers to eliminate per-frame table allocation GC overhead.
- **Cooperative Meshing Budget (`BuildBudget.lua`)**: Added fast early-exit checks and counter-based sampling for system clock reads during tile processing, cutting C-FFI polling overhead during chunk builds.
- **Vertex Sinks & Quad Generation (`ChunkMesher.lua`)**: Reused pre-allocated scratch vertex/UV array buffers (`scratchC`, `scratchUv`) inside `topQuad` and `sideQuad` to eliminate tens of thousands of temporary Lua arrays per chunk build.
- **Billboard & Figure Transforms (`VoxelScene.lua`, `ShadowMap.lua`)**: Refactored character billboard matrix operations, figure transforms, and `ShadowMap.snug` calculations to use in-place matrix operations.
- **Shader Uniform State Caching (`Voxel3D.lua`)**: Cached vertex pull uniforms to skip redundant `shader:send()` GPU driver calls per draw.

## 1.1.1 — 2026-08-08

### Changed

- **POTATO rung upscaler: sharp-bilinear → pixel_aa_fast.** The 33% rung
  now folds with fishku's pixel_aa_fast (CC0, libretro glsl-shaders
  pixel-art-scaling) instead of sharp-bilinear-simple: same one-fetch
  cost, +15% edge energy (mean 39.3 → 45.2) and anti-aliased transitions,
  zero overshoot. Selected by the bake-off over the full/single-pass
  Pixel AA variants on ALU (their slopestep `pow()` is slow on PowerVR).
  `DS_UPSCALE=sharpbilin|paa|paa_sp` retains the alternatives for A/B.

## 1.1.0 — 2026-08-08

### Added

- **RCAS upscaler on the MEDIUM/LOW fold.** LOW (50%) and MEDIUM (75%)
  keep the box fold and are sharpened at native res by AMD's RCAS
  (FidelityFX Super Resolution 1.0, MIT).
- **Pixel AA upscale on POTATO.** The 33% rung folds straight back up
  with "pixel_aa_fast" (fishku, CC0/public domain, libretro
  glsl-shaders pixel-art-scaling) -- a one-fetch anti-aliased upscaler
  that keeps the pixel grid crisp and smooths the transitions, with no
  overshoot. It won the bake-off over sharp-bilinear (+15% edge energy,
  mean 39.3 -> 45.2, zero overshoot both) at the same 1-fetch cost, and
  over the pixel_aa full/single-pass variants on ALU (the full one's
  slopestep pow() is the one op PowerVR executes slowly). It replaced the
  Catmull-Rom bicubic (which overshot/ringed at 3x -- "warped pixels")
  and made RCAS redundant on this rung, so POTATO runs upscale-only:
  crisper than the box fold, cheaper than the bicubic path it replaces.
  `DS_UPSCALE=sharpbilin|paa|paa_sp` selects the retained A/B
  alternatives. Ported in `lib/Upscale.lua`.
- **POTATO VOXEL rung** below LOW: the diorama at a third of the window
  resolution (0.33) — 2.3x less scene fill than LOW. The ladder is now
  OFF / HIGH / MEDIUM / LOW / POTATO, and the lowest two rungs stay
  flat-lit (no shadow pass), as LOW has been since brick.17.

### Changed

- The shadow-off gate covers the bottom two rungs
  (`VoxelScene`: `level >= MAX_LEVEL - 1`) so the new rung inherits the
  flat-lit treatment without re-enabling shadows on LOW.
- Bench tooling: `fix_bench_options.lua` pins the `modProfiles` enabled
  lists too (the profile system, not just the legacy `mods` table);
  probe identities must not carry stale installed copies of the mod
  under test (they shadow the checkout mount).

## 1.0.0 — 2026-08-08

### Changed

- **Fork rebranded to PotatoVoxel.** `dramatic_shape_brick` (the TrimUI
  Brick build of Dramatic Shape) is now **PotatoVoxel**: manifest id
  `potato_voxel`, display name "PotatoVoxel". The low-end tuning stays
  the default everywhere; the full desktop mod stays one env var away
  (`DS_BRICK=0`).
- Non-functional files removed: `IDEA.md`, `assets/docs/` (authoring
  methodology, the building catalogue and the two authoring scripts),
  `assets/vr/` (the OpenXR loader DLL + its Apache license — VR now needs
  the loader restored from upstream to work), `lib/ImageCache.lua`
  (unreferenced). Archived on the dev machine at
  `~/dev/archive/dramatic_shape_brick_removed/`.
- Disk mesh cache moved to `mod-derived/potato_voxel/meshes`; option row
  ids are now `potato_voxel:*` (stored values from the old ids fall back
  to defaults — everything is pinned on the potato profile anyway).
- `dramatic_shape_brick` added to the manifest conflict list, so a stale
  installed copy cannot double-render with the renamed mod.

### Fixed

- README/mod.card now credit DramaticShape as the upstream this mod is a
  fork of (they previously credited only pret/\* and Khronos).

## 1.6.2-brick.17

### Changed

- **Shadows disabled on the LOW voxel rung** (brick profile). The sun
  pass is the biggest fill-rate cost after the scene itself, and LOW is
  the rung the half-res budget bites hardest on — so the brick's LOW
  rung now renders flat-lit: no sun pass, no shadow map, no flat decal
  fallback (a decal is a shadow too). HIGH/MEDIUM keep the full shadow
  system. Off the Brick (DS_BRICK=0) shadows are unchanged on every
  rung. Measured: LOW now runs 0 shadow begins and 0 shadow draws per
  frame (was 1.0–2.0 begins/frame).

### Fixed

- **brick.16's seam-tree fix never reached the game: the mesh cache
  kept serving the old sliced geometry.** The atomic stamp-emission
  change (brick.16) altered geometry output but `GEOMETRY_VERSION`
  stayed 12, so the cache fingerprint matched and every map kept its
  pre-fix meshes — the user still saw cut trees at the transition line
  (ROUTE_2, saved at (9,71) facing the VIRIDIAN_CITY seam). Bumped to
  **13**, which forces a one-time rebuild of all meshes. Verified:
  ROUTE_2 as the current map now audits 1342 whole / 384 buried / **0
  straddling** stamps.
...
- **Border trees cut in half at the map transition line.** A tree whose
  round stamp straddled a neighbour body's edge (the seam) used to be
  culled quad by quad: quads under the neighbour's body were dropped,
  the rest kept — leaving every seam tree looking sliced, with the
  neighbour-side half missing (and, on the neighbour's own build, a
  floating half-canopy over its ground with no trunk). Trees are now
  atomic: the trunk (the stamp centre) decides, so a tree whose trunk
  is on this map's side keeps the whole stamp — canopy overhang into
  the neighbour's airspace included, which is what a tree at a road
  edge does and what the depth buffer already sorts — and a tree whose
  trunk sits under the neighbour body is dropped whole (it would rise
  through the neighbour's flat ground). The per-quad slice path is gone
  from the stamp loop entirely. Measured on Route 3's neighbours:
  ROUTE_2 71, PEWTER_CITY 21, CERULEAN_CITY 11, ROUTE_4 3 seam trees
  restored whole (the current map's own ring was already clean).

## 1.6.2-brick.15

### Changed

- **FOREST FX pinned to OFF on the Brick** (was LOW). The Viridian Forest
  volumetric god rays and ground haze were the mod's single most expensive
  per-pixel pass (FULL measured near 40% of frame cost in forests), and the
  LOW rung's beam march still cost the GE8300 more than the frame budget
  the fork exists to protect — so the atmosphere is now fully disabled by
  default, with the FOREST FX row removed from the menu with the rest of
  the pinned knobs. Off the Brick (DS_BRICK=0) the upstream FOREST FX
  ladder (FULL / LOW / OFF) is unchanged.
- **WATER pinned to OFF on the Brick** (was SKY). The reflective pass is
  the biggest fill-rate cost after shadows, and the SKY rung's mirror
  still re-drew the whole water surface every frame; a handheld diorama
  reads fine with flat water, so the row now defaults to OFF (and the
  reflective pass never runs) on the Brick. Off the Brick (DS_BRICK=0)
  the upstream WATER ladder (FULL / SKY / OFF, default FULL) is unchanged.
- **Shadow map split into WORLD + SPRITE layers.** The sun pass used to
  re-rasterise the whole world (terrain, neighbours, water, flowers,
  figures) from the sun EVERY frame a posed sprite animated — including
  the player's idle bob, so standing still (or in a menu/dialog) redrew
  the entire 1024² depth pass per frame. The world layer now redraws only
  when the camera, the sun or the world moves; the sprite layer (the few
  character/battle cards) redraws alone when a sprite animates. The main
  pass ANDs the two layers' depth tests, which is exactly the single-map
  result (`step(z, min(a,b)) == step(z,a)·step(z,b)`), so shadows land
  identically — the win is purely fewer sun-pass redraws while idle.
  Water and forest beams sample the world layer only (sprites never
  shaded water anyway, by the blue-channel decline).

## 1.6.2-brick.14

### Fixed

- **FPS drop when entering a new area (the crossing hitch).** Two
  compounding causes, both measured with a scripted transition bench
  (Route 3 → Route 4, cold cache):
  - The destination's FULL mesh was requested URGENT on crossing even
    when its BODY was already warm (prefetched as a neighbour at idle).
    The body stands in via the existing fallback, so the full now cooks
    at idle priority and the crossing never spends the urgent 10ms
    slice on visible frames. Cold maps (first voxel frame, unseen
    warps) still get the urgent full.
  - The cooperative build budget had blind spots: several heavy loops
    ran with NO `Budget.tick()` (buildVolume's column scan,
    Buildings.stamp's quad copy, roundTemplate's mask/ground-score
    passes, the region/cluster/post/relief queues, flattenQuads, the
    cache encode loops), and `Budget.tick()` itself only read the clock
    every 32nd call (then every 8th) — so a 4ms idle slice could run
    12–97ms of hull/billboard/buildings work in one resume. tick() now
    checks every call (love.timer.getTime is ~50ns) and the blind loops
    are ticked, so every resume stays inside its slice.
  - Measured transition segment: avg 8.28 → 3.43ms, p95 21.1 → 8.2ms,
    worst 87.4 → 15.7ms, frames over 16ms 19 → 0.

## 1.6.2-brick.13

### Changed

- **Aux meshes (grass/flowers/figures) are now indexed too.** They were
  the last unindexed 6-vertex-per-quad streams — the same 33% duplicated
  corners brick.11 removed from terrain/water, still wasted on every
  Route-sized aux cache (~4.5MB on Route 3). `flattenQuads` now emits
  4 unique verts per quad plus a 0-based u32 vertex map, and the aux
  cache payloads carry the index section like the terrain ones. Mesh
  cache version bumped 11 → 12 (one-time rebuild, then warm).
- **One shared upload path.** `newFfiSink.finish()` and `meshFromData()`
  ran the same sliced-upload + u32 vertex-map code as separate copies;
  both now call a single `uploadMesh()` helper.

### Fixed

- **Grass/flowers/figures vanished on a cold cache.** `meshFromData()`
  only accepted cache-load records (`ptr`/`iptr`) and returned nil for
  the fresh-build records (`buf`/`idx`) — so the very first build after
  a cache wipe silently cached the aux slots as `false` until a restart
  loaded the written files. It now accepts both record shapes.

## 1.6.2-brick.12

### Fixed

- **Indexed meshes rendered as grey mush (brick.11 regression).** LOVE's
  `Mesh:setVertexMap` convention differs by input type: a Lua TABLE map
  is 1-based, but a Data object map is RAW 0-BASED GPU convention (no
  conversion). The brick.11 sink wrote 1-based indices into the u32
  Data map, so every triangle referenced vertices one slot too far and
  the whole scene glitched into grey garbage. The sink now writes
  0-based indices, matching what LOVE's Data path expects (verified by
  a read-back probe on both the dev and app LOVE binaries). Mesh cache
  version bumped 10 → 11 so the broken-index cache files from brick.11
  are discarded. Pixels are identical to brick.10.

## 1.6.2-brick.11

### Changed

- **Indexed meshes — 33% fewer vertices, zero visual change.** The
  runtime FFI sink previously wrote every quad as 6 unindexed vertices
  (two triangles with duplicated corners). It now writes 4 unique
  vertices per quad plus a u32 vertex map — the same indexed layout the
  table path already used. A route-sized mesh drops from ~1.33M to
  ~0.89M vertices (Route 12): less GPU vertex work, less memory, faster
  uploads, smaller cache files. Winding is preserved exactly, so the
  rendered pixels are byte-identical to brick.10. Terrain and water
  cache payloads gain the index section (`encodeIndexed`/`decodeIndexed`);
  aux payloads (grass/flowers/figures) keep the unindexed layout.
  Mesh cache version bumped 9 → 10 (one-time rebuild, then warm).

## 1.6.2-brick.10

### Changed

- **Reverted the 360° sprite fan — trees are back to the crosshair
  dome.** The fan's extra planes only darkened the canopy interior
  (measured: the silhouette is byte-identical between 4 and 8 planes)
  while tripling the quad cost (Route 12 worst case 541K vs 222K), so
  the brick returns to `BILLBOARD_CROSS`: the flat card plus ±45° arms
  (~36 quads a tree). Mesh cache version bumped 8 → 9 so fan meshes are
  discarded.
...
- **The crosshair becomes a full 360° sprite fan.** Every billboard
  hull is now 8 vertical planes through its axis at azimuths k·22.5° —
  a star in plan, the classic N64-era tree (Mario 64 palms, Zelda's
  woods). Plane 0 is the flat card, byte-identical; the near-front
  planes carry the visible canopy, the back planes only add interior
  shading density. Shade falls off with azimuth (east faces the
  southeast sun and holds brightness, west sinks to the side shade).
  Measured on real Route 3 data: the treeline top edge now rises higher
  in the frame (mean row 5.1 vs 2.3 for the crosshair) with a taller,
  more varied silhouette (26px span vs 22px), and the canopy band shows
  12 shade reversals vs 7 — the densest, most 3D-looking trees yet.
  Cost: 8× the flat card (~96 quads a tree), still ~30× cheaper than
  the carve. Mesh cache version bumped 7 → 8. Off the Brick nothing
  changes (the full carve never reads the field).
- **4 planes vs 8 planes**: the silhouette is byte-identical between
  them — the extra planes only darken the canopy interior (3% of frame
  pixels). If the GE8300 ever complains, `BILLBOARD_ARMS = 4` in
  BrickProfile halves the cost with the same outline.

## 1.6.2-brick.8

### Changed

- **The sprite stack becomes a sprite CROSSHAIR.** Instead of stepping
  cards in depth, each billboard card is repeated at +45° and −45°
  about the hull's vertical axis — an X cross in plan, the classic
  crossed-billboard dome (the N64-era tree look). A true 90° cross arm
  would be edge-on to the Brick's yaw-locked south camera and invisible;
  the 45° arms project at 70.7% width and their far-side top edges rise
  above the front card, giving the canopy slanted, domed shoulders
  instead of a flat top or stacked steps. The +45 arm faces the
  southeast sun and wears a near-front shade; the −45 arm faces away and
  wears the side shade — the lit/unlit pair is what reads as a rounded
  canopy. Cost unchanged: 3 cards per hull (~36 quads a tree), still
  ~80× cheaper than the carve. Mesh cache version bumped 6 → 7. Off the
  Brick nothing changes (the full carve never reads the field).

## 1.6.2-brick.7

### Changed

- **Trees get sprite stacking — the canopy reads as layered foliage
  instead of a paper cutout.** On the Brick every billboard hull (tree,
  bush, round prop) is now three south-facing cards stepped 4px apart in
  depth (scaled to the hull's canvas, so 2x2 canopy groups stack as deep
  as single cells). The fixed 35° camera sees each back layer's top edge
  project a step higher on screen, so a tree gains visible depth while
  its silhouette and footprint stay exactly the front card's own; back
  layers shade progressively darker so the steps read as foliage mass.
  Cost: 3× the flat card (~36 quads a tree) — still ~80× cheaper than
  the carved hull. Mesh cache version bumped 5 → 6 so stacked meshes
  replace flat-card ones everywhere. Off the Brick nothing changes: the
  full carve never reads the layer fields (defaults: 1 layer, step 0).

## 1.6.2-brick.6

### Fixed

- **The void past every map edge is gone — the border forest is back.**
  On the Brick the fork had collapsed the ring of carved tree hulls to
  `ROUND_RING = 0` (the carved hulls pushed the GE8300's ~1GB shared
  memory over the edge and SIGSEGV'd the load path), which left open sky
  past the edge of every route — worst on long thin maps like Route 3.
  Billboard hulls changed the economics: a ring tree costs ~12 quads
  instead of ~3000, so the full 3-block border forest (12 tiles — the
  same width the flat renderer's ring draws) returns as dense,
  overlapping, south-facing tree cards. Worst measured (ROUTE_12) ~94K
  quads, in line with the billboard body meshes the Brick already
  renders. Mesh cache version bumped 4 → 5 so no ring-less cached mesh
  is ever served again.

## 1.6.2-brick.5

### Added

- **DEBUG row on the OPTIONS menu — the realtime benchmark panel.** ON
  arms the same instrumentation the bench driver uses (the Perf.wrap
  spans over Structures.forMap / Buildings.build / ChunkMesher.pump /
  VoxelScene.render, plus the once-per-rendered-frame stamp) and draws a
  live overlay: frame count, avg, p50, p95, p99, worst, the >16.7ms /
  >33.3ms hitch counts, the four mesh-build spans with call counts and
  totals, and the frame's draw-call / shader-switch totals. The row is the
  one that survives the Brick profile's collapse (it is instrumentation,
  not a quality knob) and the off path is free — nothing is wrapped,
  timed or drawn while it is off. Under a DS_PERF driver run the driver
  keeps ownership of the instrumentation; the panel only draws, so it can
  never skew the numbers a benchmark is measuring.

### Fixed

- **`render.hud` consumers received a nil viewport.** The mod's own
  `BattleExit` and `DayTint` wraps around `Renderer:endFrame` called the
  inner function but dropped its return value, so `Game:draw` handed every
  `render.hud` subscriber `nil` — the DEBUG panel (and any other mod's
  screen-space HUD) silently drew nothing. Both wraps now pass the
  viewport through; the panel also falls back to the window size if a
  viewport is ever missing.

## 1.6.2-brick.4

### Changed

- **The Brick profile is now the default on every device.** The fork IS
  the Brick build, so a fresh install gets the tuned profile everywhere;
  `DS_BRICK=0` in the environment is the escape hatch to the full desktop
  mod (the parent's 8-rung VOXEL ladder, the T-SHIFT pipeline, every
  OPTIONS row), `DS_BRICK=1` re-asserts the profile where a `0` is set.
- **VOXEL is a quality ladder, not a toggle.** The Brick's VOXEL row now
  reads **OFF / HIGH / MEDIUM / LOW**: HIGH renders the scene at the
  window's own resolution (1.0), MEDIUM at 3/4 (0.75), LOW at half (0.5 —
  the original tuned rung). Every on-rung is the same 35-degree diorama
  framing; the rung only changes how many pixels the GE8300 fills, so the
  frame budget is now a choice rather than a fixed 0.5.

## 1.6.2-brick.3

### Added

- **Brick profile forceable on any device.** `DS_BRICK=1` applies the
  Brick tuning (OFF/ON VOXEL, WATER=SKY, FOREST FX=LOW, 3D-BTL off, AA
  off, half-resolution scene render, low-poly billboard terrain) on any
  platform; `DS_BRICK=0` forces it off even on Linux. Unset: auto-detect
  (Linux/Brick only), so off-device behaviour stays byte-for-byte
  upstream. The billboard hulls assume the Brick's fixed south-facing
  orbit camera — see the README camera caveat before forcing on a
  free-orbit desktop.
- **Disk mesh cache active on every device.** The pre-built mesh cache
  (previously Brick-only, via the portable SD-card folder) now also runs
  on desktops and other handhelds, storing under the LÖVE save directory
  when no portable root exists. The fingerprint gained a brick/full
  profile token (geometry version 4), so a brick-built mesh is never
  served to full mode or vice-versa; stale, corrupt or cross-profile
  files still silently fall back to a normal rebuild.

### Fixed

- **The cache never survived a restart.** `Assets.register`'s invalidator
  fired `ChunkMesher.invalidate()` (no map id) on every boot's loader
  install, and that wiped the whole disk cache — every launch was cold
  (the ~60s 2D→diorama build, every time, on the Brick too). The full
  wipe now drops memory only; disk files are fingerprint-protected and
  self-invalidate at load, so a warm launch loads the previous session's
  meshes instead of rebuilding them (bench: pump work 261ms → 19ms,
  Structures analysis 2 calls → 0).
- **Aux cache write read 6× past its buffer (crash).** `flattenQuads`
  returns a float count; the cache payloads are vertex-counted. The aux
  save fed floats in as vertices, so the encoder read `n*6` floats from
  an `n`-float buffer — a native SIGSEGV past the FFI allocation at
  Viridian's grass scale (caught by the desktop bench; it would have hit
  the Brick's first large-map cache write too).
- **Aux cache could never load.** `decodeMesh` demanded an exact payload
  length, but aux files concatenate grass, flowers and figures payloads
  — every multi-payload file failed validation and the aux cache missed
  100% of the time, silently rebuilding grass/flowers each visit. Now a
  minimum-length (truncation) guard.
- **Void-fill changes now invalidate via the fingerprint** (the border
  ring is baked into the mesh): the setting rides the cache key, so a
  change misses cleanly instead of nuking every cached map.
- Both aux bugs covered by a new headless aux round-trip test at 10,000
  quads: byte-exact file size and byte-identical stream (the old headless
  suite only round-tripped terrain, which is why both shipped).

## 1.6.2-brick.2

### Fixed

- Keep the standard player's back-facing battle sprite unless the explicit
  staged 3D battle path is active; this prevents the lower-left battler from
  being replaced by front-facing art in ordinary battles.

## 1.6.2-brick.1

### Added

- **TrimUI Brick build — a single ON / OFF toggle.** On the Brick the mod
  is exactly two rungs, **VOXEL OFF / ON** (hotkey `3` or SELECT), with
  every quality knob pre-tuned for the device (tg5040: 4x Cortex-A53
  @1.8GHz, PowerVR GE8300, ~998MB RAM):
  - WATER pinned to SKY (the reflection without the per-pixel depth ray
    march).
  - FOREST FX pinned to LOW (keeps the volumetric shafts, drops the
    densest scatter).
  - 3D-BTL pinned to OFF (a battle's 3D staging was the single biggest
    mid-battle cost). Flat card battles on the diorama floor.
  - AA pinned to OFF -- the 4X rung's 4x-pixel canvases are not worth a
    handheld's fill rate and RAM.
  - V-CURVE and V-GRID pinned to OFF -- ornaments a fixed diorama does not
    need.
  - ChunkMesher build budgets halved (urgent 12ms->5ms, idle 5ms->2ms,
    covered 30ms->15ms): the mesh pump yields sooner so carve-in
    animation stays responsive on four cores.
  - The mod's own OPTIONS rows (T-SHIFT, WATER, 3D-BTL, BACK SPRITES, AA,
    V-GRID, V-CURVE, VR, HORDE) are removed from the menu and the manager
    page; only the VOXEL OFF/ON row remains. The engine's own rows keep
    their usual behavior.
- Applied at runtime by `lib/BrickProfile.lua` (a no-op on any other
  platform), so off the Brick the fork behaves exactly like upstream
  1.6.2 with every setting rung intact.
